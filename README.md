...
- Fixed recommendation enrichment by using OMDb (movies) and TVMaze (TV) instead of unauthenticated TMDB endpoints
- Better posters via a curated poster map with fallbacks; corrected broken poster URLs and title mismatches
- Stable sharing links using configurable SITE_URL
- Service Worker hardened (no localStorage usage in SW context)
- Added `vercel.json` for optimal static hosting and security headers
...
`vercel.json` is included and configured for a static site with safe headers and long-term asset caching.
...
- Base recommendations are selected from a curated database by mood and time.
- We enhance results live using:
  - OMDb (movies) – ratings, posters, plot, runtime.
  - TVMaze (TV) – images, ratings, metadata.
  - Best-effort TMDB read-only endpoints for images when available.
- Known-popular titles use a curated poster map for fast, reliable images.
