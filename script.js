...
// OMDb API (Free key supported)
const OMDbApi = {
    get apiKey() {
        return (typeof CONFIG !== 'undefined' && CONFIG.OMDB_API_KEY) ? CONFIG.OMDB_API_KEY : null;
    },
    get baseUrl() {
        return (typeof CONFIG !== 'undefined' && CONFIG.OMDB_BASE_URL) ? CONFIG.OMDB_BASE_URL : 'https://www.omdbapi.com';
    },
    async fetchByTitle(title) {
        if (!this.apiKey) return null;
        try {
            const res = await fetch(`${this.baseUrl}/?apikey=${this.apiKey}&t=${encodeURIComponent(title)}&plot=short&r=json`);
    ...
    async enhanceShowData(show) {
        ...
        if (result.type === 'Movie') {
            try {
                ...
                if (omdb) {
                    result.image = (omdb.Poster && omdb.Poster !== 'N/A') ? omdb.Poster : result.image;
                    result.rating = omdb.imdbRating && omdb.imdbRating !== 'N/A' ? omdb.imdbRating : result.rating;
                    result.year = omdb.Year || result.year;
                    result.runtime = omdb.Runtime || result.runtime;
                    result.description = omdb.Plot && omdb.Plot !== 'N/A' ? omdb.Plot : result.description;
                }
            } catch (error) {
                console.warn('OMDb enhancement failed:', error);
            }
        }
        ...
        if (result.type === 'TV Series' || result.type === 'TV Episode' || result.type === 'Documentary Episode') {
            try {
                ...
                if (tvmazeResult) result = tvmazeResult;
            } catch (error) {
...
// Pure function to generate recommendations for a given mood and time (testable)
function generateRecommendationsFor(mood, time) {
    if (!mood || !time) return [];
    const categoryShows = showsDatabase[mood]?.[time] || [];
    const shuffled = [...categoryShows].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(3, shuffled.length));
}

// Generate recommendations based on currently selected mood and time (UI)
function generateRecommendations() {
    return generateRecommendationsFor(selectedMood, selectedTime);
}
...
function initializeSharing() {
    ...
    shareBtn.addEventListener('click', () => {
        ...
        const siteUrl = (typeof CONFIG !== 'undefined' && CONFIG.SITE_URL) ? CONFIG.SITE_URL : window.location.origin;
...
// Initialize all functionality when DOM is loaded (guarded for Node tests)
...
// Utility to detect placeholder URLs
function isPlaceholderUrl(url) {
    return !url || url.includes('via.placeholder.com');
}

// Get poster from CONFIG mapping (case-insensitive)
function getPosterFromConfig(title) {
    try {
        if (typeof CONFIG === 'undefined' || !CONFIG.MOVIE_POSTERS) return null;
        const entries = Object.entries(CONFIG.MOVIE_POSTERS);
        const found = entries.find(([key]) => key.toLowerCase() === title.toLowerCase());
        return found ? found[1] : null;
    } catch (_) { return null; }
}

// Service Worker registration for PWA capabilities (guarded for Node tests)
...
// Intersection Observer for animations (guarded for Node tests)
